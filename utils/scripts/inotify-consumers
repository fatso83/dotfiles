...
generateData(){
    ...
    find /proc/*/fd -lname anon_inode:inotify 2> /dev/null |
        ...
        uniq |
        {
            while read -rs PROC; do      # count watchers of processes found
                echo "${PROC},$(grep -c "^inotify" < <(cat /proc/${PROC}/fdinfo/* 2> /dev/null))"
            done
        } |
        grep -v ",0$" |                  # remove entires without watchers
        sort -n -t "," -k 2 -r |         # sort to begin with highest numbers
        {                                # group commands so that $TOT is visible in the printf
            while read -rs PID CNT; do   # show watchers and corresponding process info
                printf "%$(( WLEN - 2 ))d  %s\n" "$CNT" "$(grep -e "^\ *${PID}\ " <<< "$PSLIST")"
                TOT=$(( TOT + CNT ))
            done
            printf "\n%$(( WLEN - 2 ))d  %s\n" "$TOT" "WATCHERS TOTAL COUNT"
        }
}
